from __future__ import annotations

import asyncio
import difflib
import logging
import random
from collections.abc import Callable
from functools import partial, wraps
from typing import TYPE_CHECKING

# import for type inference
import spotipy
from homeassistant.components.cast.helpers import ChromecastInfo
from homeassistant.components.cast.media_player import CastDevice
from homeassistant.components.spotify.media_player import SpotifyMediaPlayer
from homeassistant.exceptions import HomeAssistantError
from homeassistant.helpers import entity_platform

if TYPE_CHECKING:
    from homeassistant.core import HomeAssistant

_LOGGER = logging.getLogger(__name__)


def get_spotify_media_player(hass: HomeAssistant, spotify_user_id: str) -> SpotifyMediaPlayer:
    """Get the spotify media player entity from hass."""
    platforms = entity_platform.async_get_platforms(hass, "spotify")
    spotify_media_player = None
    for platform in platforms:
        if platform.domain != "media_player":
            continue

        for entity in platform.entities.values():
            if (
                isinstance(entity, SpotifyMediaPlayer)
                and entity.unique_id == spotify_user_id
            ):
                try:
                    entity_devices = entity._devices  # noqa: SLF001
                except AttributeError:
                    entity_devices = entity.data.devices.data

                _LOGGER.debug("get_spotify_devices: %s: %s: %s", entity.entity_id, entity.name, entity_devices)
                spotify_media_player = entity
                break

    if spotify_media_player:
        return spotify_media_player
    else:
        raise HomeAssistantError("Could not find spotify media player.")


def get_spotify_devices(spotify_media_player: SpotifyMediaPlayer) -> dict:
    if spotify_media_player:
        # Need to come from media_player spotify's sp client due to token issues
        try:
            spotify_devices = spotify_media_player._spotify.devices()  # noqa: SLF001
        except AttributeError:
            spotify_devices = spotify_media_player.data.client.devices()

        _LOGGER.debug("get_spotify_devices: %s", spotify_devices)

        return spotify_devices
    return []


def get_spotify_install_status(hass: HomeAssistant) -> bool:

    platform_string = "spotify"
    platforms = entity_platform.async_get_platforms(hass, platform_string)
    platform_count = len(platforms)

    if platform_count == 0:
        _LOGGER.error("%s integration not found", platform_string)
    else:
        _LOGGER.debug("%s integration found", platform_string)

    return platform_count != 0


def get_cast_devices(hass: HomeAssistant) -> list[ChromecastInfo]:
    platforms = entity_platform.async_get_platforms(hass, "cast")
    cast_infos = []
    for platform in platforms:
        if platform.domain != "media_player":
            continue
        for entity in platform.entities.values():
            if isinstance(entity, CastDevice):
                _LOGGER.debug(
                    "get_cast_devices: %s: %s cast info: %s",
                    entity.entity_id,
                    entity.name,
                    entity._cast_info,
                )
                cast_infos.append(entity._cast_info)
    return cast_infos


# Async wrap sync function
def async_wrap(func) -> Callable:
    @wraps(func)
    async def run(*args, loop=None, executor=None, **kwargs):
        if loop is None:
            loop = asyncio.get_event_loop()
        pfunc = partial(func, *args, **kwargs)
        return await loop.run_in_executor(executor, pfunc)

    return run


def get_search_results(search: str, spotify_client: spotipy.Spotify, country: str | None = None) -> str:

    _LOGGER.debug("using search query to find uri")
    search = search.upper()
    results = []

    for search_type in ["artist", "album", "track", "playlist"]:
        result = spotify_client.search(
            f"{search_type}:{search}",
            limit=1,
            offset=0,
            type=search_type,
            market=country,
        ).get(f"{search_type}s")
        if result:
            item = result['items'][0]
            results.append(
                {
                    'name': item['name'].upper(),
                    'uri': item['uri'],
                }
            )
            _LOGGER.debug("search result for %s: %s", search_type, item['name'])

    best_match = sorted(
        results,
        key=lambda x: difflib.SequenceMatcher(None, x['name'], search).ratio(),
        reverse=True,
    )[0]

    _LOGGER.debug("Best match for %s is %s", search, best_match['name'])
    return best_match['uri']


def get_random_playlist_from_category(
    spotify_client: spotipy.Spotify,
    category: str,
    country: str | None = None,
    limit: int = 20,
) -> str | None:

    if country is None:
        _LOGGER.debug(
            "Get random playlist among %d playlists from category %s, no country specified.",
            limit,
            category,
        )
    else:
        _LOGGER.debug(
            "Get random playlist among %d playlists from category %s in country %s",
            limit,
            category,
            country,
        )

        # validate category and country are valid entries
        if country.upper() not in spotify_client.country_codes:
            _LOGGER.error("%s is not a valid country code", country)
            return None

    # get list of playlist from category and localisation provided
    try:
        playlists = spotify_client.category_playlists(
            category_id=category,
            country=country,
            limit=limit,
        )["playlists"]["items"]
    except spotipy.exceptions.SpotifyException as e:
        _LOGGER.exception(e.msg)
        return None

    # choose one at random
    chosen = random.choice(playlists)
    _LOGGER.debug("Chose playlist %s (%s) from category %s.", chosen['name'], chosen['uri'], category)
    return chosen['uri']


def is_valid_uri(uri: str) -> bool:

    # list of possible types
    types = [
        "artist",
        "album",
        "track",
        "playlist",
        "show",
        "episode"
    ]

    # split the string
    elems = uri.split(":")

    # validate number of sub elements
    if elems[1].lower() == "user":
        elems = elems[0:1] + elems[3:]
        types = ["playlist"]
        _LOGGER.debug("Excluding user information from the Spotify URI validation. Only supported for playlists")

    # support playing a user's liked songs list (spotify:user:username:collection)
    if len(elems) == 2 and elems[1].lower() == "collection":
        return True

    if len(elems) != 3:
        _LOGGER.error("[%s] is not a valid URI. The format should be [spotify:<type>:<unique_id>]", uri)
        return False

    # check correct format of the sub elements
    if elems[0].lower() != "spotify":
        _LOGGER.error(
            "This is not a valid Spotify URI. This should start with [spotify], but instead starts with [%s]",
            elems[0],
        )
        return False

    if elems[1].lower() not in types:
        _LOGGER.error(
            "%s is not a valid type for Spotify request. Please make sure to use the following list %s",
            elems[1],
            types,
        )
        return False

    if "?" in elems[2]:
        _LOGGER.warning(
            "%s contains query character. This should work, but you should probably remove it and anything after.",
            elems[2],
        )

    return True


def is_empty_str(string: str) -> bool:
    return string is None or string.strip() == ""
